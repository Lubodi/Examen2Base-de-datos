...
router.get('/', async (req, res) => {
    try {
        ...
        const queryResult = await req.connection.query('SELECT * FROM hr.JOB_HISTORY;');

        // Transforma el recordset a un array de arrays
        const transformedResult = queryResult.recordset.map(row => [row.EMPLOYEE_ID, row.START_DATE, row.END_DATE, row.JOB_ID, row.DEPARMENT_ID]);
